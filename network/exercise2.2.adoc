...
:license_url: http://ansible.redhatgov.io/workshop-files/ansible_tower_license.txt
...
workshop, we are using a *network* credential.
...
----
*Step 6:* Use the ```tower-manage``` command to import an existing inventory.  (_Be sure to replace student(X) with your student number_)
----
...
image::at_inv_group_detail.png[caption="Figure 10: ",title="routers inventory group detail",link="{image_links}/at_inv_group_detail.png"]
