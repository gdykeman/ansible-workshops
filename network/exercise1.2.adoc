...
Next, we are using the debug module to display the output of what information is actually captured when using the *ios_facts* module.
...
- name: Backup configuration
  ios_config:
    backup: yes
----
- These three lines are calling the Ansible module *ios_config* and passing in *backup: yes* to capture the configuration of the routers and generate a backup file.
